<template>
  <div class="subdetails">
    <h3 class="head">{{list.title || list.type}}</h3>
    ...
    <swiper class="swiper" :indicator-dots="false" easing-function="easeInOutCubic" :circular="true" duration="1000">
      <block v-for="(item, index) in list.content" :index="index" :key="index">
        <swiper-item class="swiper-item">
          <h3 class="chapter">{{item.chapter}}</h3>
          <p class="author">{{item.author}}</p>
          <ul >
            <li class="paragraph" v-for="(v,k) in item.paragraphs" :key="k">{{v}}</li>
          </ul>
        </swiper-item>
      </block>
    </swiper>
